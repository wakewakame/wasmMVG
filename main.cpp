...
SfM_Data sceneToSfM_Data(const Scene& scene) {
	SfM_Data sfm_data;
	for(const std::pair<size_t, Camera> &camera : scene.cameras) {
	...
	for(const std::pair<size_t, Point> &point : scene.points) {
		openMVG::sfm::Observations obs;
		for(const std::pair<size_t, Vec2> &camera : point.second.cameras) {
...
Scene SfM_DataToScene(SfM_Data &sfm_data) {
	Scene scene;
	for(const std::pair<size_t, std::shared_ptr<openMVG::sfm::View>> &view : sfm_data.views) {
		Camera &camera = scene.cameras[view.first];
		camera.pose = sfm_data.poses[view.second->id_pose];
		camera.intrinsic = sfm_data.intrinsics[view.second->id_intrinsic];
	}
	for(const std::pair<size_t, openMVG::sfm::Landmark> &landmark : sfm_data.structure) {
		Point &point = scene.points[landmark.first];
		for(const std::pair<size_t, openMVG::sfm::Observation> &obs : landmark.second.obs) {
...
Val sceneToVal(const Scene &scene) {
	...
	for(const std::pair<size_t, Point> &point : scene.points) {
		...
		for(const std::pair<size_t, Vec2> &point_cameras : point.second.cameras) {
